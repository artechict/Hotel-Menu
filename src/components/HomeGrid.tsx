...
export function HomeGrid({ navigateTo, t, settings }: HomeGridProps) {
  const cards = [
    { id: 'restaurant', icon: <Utensils size={32} />, label: t.restaurant, color: 'from-emerald-500 to-teal-600', delay: 0.1 },
    { id: 'cafe', icon: <Coffee size={32} />, label: t.cafe, color: 'from-orange-500 to-amber-600', delay: 0.2 },
    { id: 'laundry', icon: <Shirt size={32} />, label: t.laundry, color: 'from-blue-500 to-indigo-600', delay: 0.3 },
    { id: 'phones', icon: <Phone size={32} />, label: t.contact, color: 'from-rose-500 to-pink-600', delay: 0.4 },
    { id: 'info', icon: <Info size={32} />, label: t.info, color: 'from-zinc-700 to-zinc-900', delay: 0.5 },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {cards.map((card) => (
          <motion.button
            ...
            className={`relative overflow-hidden p-8 rounded-[2.5rem] text-left group h-48 md:h-64 flex flex-col justify-between transition-all shadow-xl hover:shadow-2xl ${card.id === 'info' ? 'col-span-2 md:col-span-1' : ''}`}
          >
            ...
            <div className="relative z-10 p-3 bg-white/20 backdrop-blur-md rounded-2xl w-fit shadow-lg border border-white/20">
              {card.icon}
            </div>
            
            <div className="relative z-10">
              <span className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/60 mb-1">Service</span>
